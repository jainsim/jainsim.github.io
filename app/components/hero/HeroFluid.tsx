 ...
 * from the pointer and drifts/dissolves through the scene; a gentle automatic
 * splat keeps the mist alive when the pointer is still.
 *
 * Rendered transparently between the hero background image and the content.
 * Respects prefers-reduced-motion (no init) and pauses while offscreen.
 */
export default function HeroFluid() {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (!gl) return { gl: null as any, ext: {} as any };
    ...
    // Gentle self-sustaining mist so the scene breathes without the pointer.
    ...
    // ---- Pointer input (vanilla, no jQuery) ----
    const onMouseMove = (e: MouseEvent) => {
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
      if (!pointer.color || pointer.color.length === 3) pointer.color = generateColor();
    ...
    // Seed a couple of soft splats so there's mist before any interaction.
    for (let i = 0; i < 3; i++) {
      splat(Math.random(), 0.4 + Math.random() * 0.4, (Math.random() - 0.5) * 200, -40, generateColor());
    }
